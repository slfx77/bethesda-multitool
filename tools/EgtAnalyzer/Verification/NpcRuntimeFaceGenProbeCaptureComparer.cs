...
internal static class NpcRuntimeFaceGenProbeCaptureComparer
{
    internal static RuntimeFaceGenProbeCapture LoadCapture(string captureDir)
    {
        ...
        var racePageACsvPath = Path.Combine(fullDir, "race_texture_coeffs_page_a.csv");
        var racePageBCsvPath = Path.Combine(fullDir, "race_texture_coeffs_page_b.csv");
        var mergedPageACsvPath = Path.Combine(fullDir, "texture_coeffs_page_a.csv");
        var mergedPageBCsvPath = Path.Combine(fullDir, "texture_coeffs_page_b.csv");
        ...
        return new RuntimeFaceGenProbeCapture
        {
            ...
            CaptureDirectoryName =
                Path.GetFileName(fullDir.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar)),
            ...
            SelectedRacePage = descriptors.TryGetProperty("selectedRacePage", out var selectedRacePage)
                ? selectedRacePage.GetString()
                : null,
            RaceTexturePageADescriptor = descriptors.TryGetProperty("raceTexturePageA", out var raceTexturePageA)
                ? ParseDescriptor(raceTexturePageA)
                : null,
            RaceTexturePageBDescriptor = descriptors.TryGetProperty("raceTexturePageB", out var raceTexturePageB)
                ? ParseDescriptor(raceTexturePageB)
                : null,
            ...
            RaceTexturePageACoefficients = LoadCoefficientCsv(racePageACsvPath),
            RaceTexturePageBCoefficients = LoadCoefficientCsv(racePageBCsvPath),
            MergedTexturePageACoefficients = LoadCoefficientCsv(mergedPageACsvPath),
            MergedTexturePageBCoefficients = LoadCoefficientCsv(mergedPageBCsvPath)
        };
    }
        ...
        string summaryPath)
    {
        ...
        sb.AppendLine(
            ...
            "runtime_page_a_match_form_id,runtime_page_a_match_editor_id,runtime_page_a_match_sex,runtime_page_a_match_relation,runtime_page_a_match_mae,runtime_page_a_match_max_abs_delta," +
            "runtime_page_b_match_form_id,runtime_page_b_match_editor_id,runtime_page_b_match_sex,runtime_page_b_match_relation,runtime_page_b_match_mae,runtime_page_b_match_max_abs_delta," +
            "best_race_match_form_id,best_race_match_editor_id,best_race_match_page,best_race_match_sex,best_race_match_relation,best_race_match_mae,best_race_match_max_abs_delta");

        foreach (var result in results.OrderBy(r => r.FormId).ThenBy(r => r.CaptureDirectoryName))
        {
            var runtimePageAMatch = FindBestRuntimePageMatch(result.RaceMatches, "a");
            var runtimePageBMatch = FindBestRuntimePageMatch(result.RaceMatches, "b");
            var bestRaceMatch = result.RaceMatches?.OrderBy(match => match.Comparison.MeanAbsoluteDelta)
                .FirstOrDefault();
            ...
            AppendRaceMatchSummary(sb, runtimePageAMatch, false);
            AppendRaceMatchSummary(sb, runtimePageBMatch, false);
            ...
            sb.Append(Csv(bestRaceMatch?.RuntimePage)).Append(',');
            sb.Append(Csv(bestRaceMatch?.CandidateSex)).Append(',');
            sb.Append(Csv(bestRaceMatch?.RelationToRuntimeRace)).Append(',');
            sb.Append(Csv(bestRaceMatch?.Comparison.MeanAbsoluteDelta.ToString("F6", CultureInfo.InvariantCulture)))
                .Append(',');
            sb.Append(Csv(bestRaceMatch?.Comparison.MaxAbsoluteDelta.ToString("F6", CultureInfo.InvariantCulture)))
                .AppendLine();
        }
    ...
    internal static RuntimeFaceGenProbeRaceMatch? FindBestRuntimePageMatch(
        IReadOnlyList<RuntimeFaceGenProbeRaceMatch>? matches,
        string runtimePage)
    {
        return matches?
            .Where(match => string.Equals(match.RuntimePage, runtimePage, StringComparison.OrdinalIgnoreCase))
            .OrderBy(match => match.Comparison.MeanAbsoluteDelta)
            .ThenBy(match => match.Comparison.MaxAbsoluteDelta)
            .ThenBy(match => match.RaceFormId)
            .ThenBy(match => match.CandidateSex, StringComparer.Ordinal)
            .FirstOrDefault();
    }
    ...
    private static RuntimeFaceGenProbeDescriptor ParseDescriptor(JsonElement element)
    {
        return new RuntimeFaceGenProbeDescriptor(
            element.TryGetProperty("descriptorAddress", out var descriptorAddress)
                ? descriptorAddress.GetString()
                : null,
        ...
        IReadOnlyDictionary<uint, RaceScanEntry>? races)
    {
        ...
        var runtimePages = EnumerateRuntimeRacePages(capture).ToList();
        if (runtimePages.Count == 0)
        {
            return [];
        }

        var matches = new List<RuntimeFaceGenProbeRaceMatch>(races.Count * 2 * runtimePages.Count);

        foreach (var runtimePage in runtimePages)
        {
            foreach (var (raceFormId, race) in races)
            {
                AddRaceMatch(matches, capture, runtimeRace, runtimePage, raceFormId, race, race.MaleFaceGenTexture,
                    false);
                AddRaceMatch(matches, capture, runtimeRace, runtimePage, raceFormId, race, race.FemaleFaceGenTexture,
                    true);
            }
        }
        ...
        return matches
            .OrderBy(match => match.Comparison.MeanAbsoluteDelta)
            .ThenBy(match => match.Comparison.MaxAbsoluteDelta)
            .ThenBy(match => match.RuntimePage, StringComparer.OrdinalIgnoreCase)
            .ThenBy(match => match.EditorId, StringComparer.OrdinalIgnoreCase)
            .ToList();
    }

    private static IEnumerable<RuntimeFaceGenProbeRuntimePage> EnumerateRuntimeRacePages(
        RuntimeFaceGenProbeCapture capture)
    {
        if (capture.RaceTexturePageACoefficients.Length > 0)
        {
            yield return new RuntimeFaceGenProbeRuntimePage("a", capture.RaceTexturePageACoefficients);
        }

        if (capture.RaceTexturePageBCoefficients.Length > 0)
        {
            yield return new RuntimeFaceGenProbeRuntimePage("b", capture.RaceTexturePageBCoefficients);
        }

        if (capture.RaceTexturePageACoefficients.Length == 0 &&
            capture.RaceTexturePageBCoefficients.Length == 0 &&
            capture.RaceTextureCoefficients.Length > 0)
        {
            yield return new RuntimeFaceGenProbeRuntimePage(capture.SelectedRacePage ?? "selected",
                capture.RaceTextureCoefficients);
        }
    }
    ...
    private static void AddRaceMatch(
        ...
        RuntimeFaceGenProbeCapture capture,
        RaceScanEntry? runtimeRace,
        RuntimeFaceGenProbeRuntimePage runtimePage,
        uint raceFormId,
        RaceScanEntry race,
        float[]? coefficients,
        bool candidateIsFemale)
    {
        ...
        matches.Add(new RuntimeFaceGenProbeRaceMatch(
            ...
            CompareArray("race_candidate", runtimePage.Coefficients, coefficients),
            FitUniformScale(runtimePage.Coefficients, coefficients)));
    }
        ...
        IReadOnlyList<RuntimeFaceGenProbeRaceMatch>? matches)
    {
        var sb = new StringBuilder();
        sb.AppendLine(
            "race_form_id,editor_id,runtime_page,candidate_sex,relation,compared_count,mean_abs_delta,rmse_delta,max_abs_delta,mean_signed_delta,exact_match_count,fit_scale,fit_mean_abs_delta,fit_rmse_delta,fit_max_abs_delta,fit_mean_signed_delta");
        ...
        if (matches != null)
        {
            foreach (var match in matches)
            {
                ...
                sb.Append(Csv(match.RuntimePage)).Append(',');
                ...
                sb.Append(match.Comparison.RootMeanSquareDelta.ToString("F6", CultureInfo.InvariantCulture))
                    .Append(',');
                ...
                sb.Append(match.Fit.Comparison.MeanAbsoluteDelta.ToString("F6", CultureInfo.InvariantCulture))
                    .Append(',');
                sb.Append(match.Fit.Comparison.RootMeanSquareDelta.ToString("F6", CultureInfo.InvariantCulture))
                    .Append(',');
                sb.Append(match.Fit.Comparison.MaxAbsoluteDelta.ToString("F6", CultureInfo.InvariantCulture))
                    .Append(',');
                sb.Append(match.Fit.Comparison.MeanSignedDelta.ToString("F6", CultureInfo.InvariantCulture))
                    .AppendLine();
            }
        }
    ...
    private static string BuildSummaryText(RuntimeFaceGenProbeComparisonResult result)
    {
        ...
        if (result.RaceMatches is { Count: > 0 })
        {
            ...
            foreach (var match in result.RaceMatches.Take(5))
            {
                sb.AppendLine(
                    CultureInfo.InvariantCulture,
                    $"  0x{match.RaceFormId:X8} {match.EditorId ?? string.Empty} page={match.RuntimePage} sex={match.CandidateSex} relation={match.RelationToRuntimeRace} mae={match.Comparison.MeanAbsoluteDelta:F6} max={match.Comparison.MaxAbsoluteDelta:F6}");
            }
            ...
            foreach (var match in result.RaceMatches.OrderBy(match => match.Fit.Comparison.MeanAbsoluteDelta).Take(5))
            {
                sb.AppendLine(
                    CultureInfo.InvariantCulture,
                    $"  0x{match.RaceFormId:X8} {match.EditorId ?? string.Empty} page={match.RuntimePage} sex={match.CandidateSex} relation={match.RelationToRuntimeRace} scale={match.Fit.Scale:F6} fitted_mae={match.Fit.Comparison.MeanAbsoluteDelta:F6} fitted_max={match.Fit.Comparison.MaxAbsoluteDelta:F6}");
            }
        }
    ...
    private static void AppendSummaryBlock(StringBuilder sb, RuntimeFaceGenProbeArrayComparison? comparison)
    {
        ...
        sb.AppendLine(
            $"{comparison.Label}_mean_abs_delta={comparison.MeanAbsoluteDelta.ToString("F6", CultureInfo.InvariantCulture)}");
        sb.AppendLine(
            $"{comparison.Label}_rmse_delta={comparison.RootMeanSquareDelta.ToString("F6", CultureInfo.InvariantCulture)}");
        sb.AppendLine(
            $"{comparison.Label}_max_abs_delta={comparison.MaxAbsoluteDelta.ToString("F6", CultureInfo.InvariantCulture)}");
        sb.AppendLine(
            $"{comparison.Label}_mean_signed_delta={comparison.MeanSignedDelta.ToString("F6", CultureInfo.InvariantCulture)}");
    ...
    private static void AppendRaceMatchSummary(
        StringBuilder sb,
        RuntimeFaceGenProbeRaceMatch? match,
        bool includeRuntimePage)
    {
        sb.Append(Csv(match?.RaceFormId.ToString("X8", CultureInfo.InvariantCulture))).Append(',');
        sb.Append(Csv(match?.EditorId)).Append(',');
        if (includeRuntimePage)
        {
            sb.Append(Csv(match?.RuntimePage)).Append(',');
        }
        ...
        sb.Append(Csv(match?.CandidateSex)).Append(',');
        sb.Append(Csv(match?.RelationToRuntimeRace)).Append(',');
        sb.Append(Csv(match?.Comparison.MeanAbsoluteDelta.ToString("F6", CultureInfo.InvariantCulture))).Append(',');
        sb.Append(Csv(match?.Comparison.MaxAbsoluteDelta.ToString("F6", CultureInfo.InvariantCulture))).Append(',');
    }
