...
public sealed record SkyMoonProfile
{
    ...
    /// <summary>
    ///     Fallback primary-moon half-extent (fraction of the billboard radius) when the engine's
    ///     authored size inputs aren't available.
    /// </summary>
    ...
    /// <summary>
    ///     Primary moon's sky orbit (distinct from <see cref="SecondaryOrbit" /> so two moons don't
    ///     share an arc). Single-moon games use only this.
    /// </summary>
    ...
    /// <summary>
    ///     Days per moon phase for this game (8 phases × this = full lunar cycle). Morrowind = 3
    ///     (24-day cycle). Only meaningful where <see cref="HasPerPhaseTextures" />.
    /// </summary>
    public int PhaseLengthDays { get; init; } = MoonSky.MorrowindPhaseLengthDays;

    /// <summary>
    ///     Day offset of the second moon's phase from the first, so the two moons aren't phase-locked
    ///     (seeded; visually calibrated against OpenMW).
    /// </summary>
    public int SecondaryPhaseOffsetDays { get; init; }

    /// <summary>
    ///     Format string for the primary moon's per-phase texture (<c>{0}</c> = a <see cref="PhaseTokens" />
    ///     entry), e.g. Morrowind's <c>textures\tx_masser_{0}.dds</c>. Null when the game ships no per-phase
    ///     moon textures — the renderer then draws the single full-moon texture for every phase.
    /// </summary>
    ...
    /// <summary>
    ///     The 8 phase tokens in phase-index order (0 = new … 4 = full … 7 = waning crescent), spliced
    ///     into the phase-texture patterns. Empty when the game has no per-phase textures.
    /// </summary>
    ...
    /// <summary>
    ///     True when the game ships distinct per-phase moon textures (Morrowind, Oblivion — both
    ///     moons — FO3/FNV's masser set, and FO4/76's secunda, all archive-verified). Skyrim shades phases
    ///     in-shader, so it reuses the single full-moon texture for every phase.
    /// </summary>
    public bool HasPerPhaseTextures =>
        PrimaryPhaseTexturePattern is not null && PhaseTokens.Count == MoonSky.PhaseCount;
        ...
        return PathFamily switch
        {
            MoonPathFamily.FalloutRotatedArm or MoonPathFamily.SkyrimRotatedArm =>
                MoonSky.ComputeRotatedArmDirection(
                    rotatedArmSpeedOverride ?? RotatedArmSpeed(secondary),
                    rotatedArmInclinationOverride ?? RotatedArmInclination(secondary),
                    gameHour,
                    day),
    ...
    /// <summary>
    ///     The per-phase texture path for a moon at <paramref name="phaseIndex" /> (0..7), or null when
    ///     this game has no per-phase textures (caller falls back to the full-moon texture). The index is
    ///     clamped into range so a bad phase can't throw.
    /// </summary>
    public string? PhaseTexturePath(bool secondary, int phaseIndex)
    {
        ...
        return string.Format(CultureInfo.InvariantCulture, pattern, PhaseTokens[idx]);
    }
    ...
    {
        return moonSizeSetting is int size && sunXExtreme is float dome && dome > 0f ? size / dome : null;
    }

    /// <summary>The moon configuration for the loaded game. Unknown/Starfield → <see cref="None" />.</summary>
    public static SkyMoonProfile ForGame(BethesdaGame game)
    {
        return game switch
        {
