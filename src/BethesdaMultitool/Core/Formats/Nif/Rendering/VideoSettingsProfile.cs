...
/// </summary>
/// <param name="HasTonemapSelector">Any post-processing selector applies (everything but
/// Morrowind, whose engine has no HDR/bloom/imagespace stage — the panel's whole tonemap UI
/// hides, matching the existing TonemapSection rule).</param>
/// <param name="HasClassicHdrTriple">The launcher's three-way None/Bloom/HDR radio
/// (<c>bDoHighDynamicRange:BlurShaderHDR</c> / <c>bUseBlurShader:BlurShader</c>) — Oblivion,
/// FO3, FNV ship the identical launcher block. Games without it show HDR/None only.</param>
/// <param name="HasWaterReflections">Water reflection toggle (<c>bUseWaterReflections:Water</c>)
/// — games whose recovered water shader consumes a reflection input.</param>
/// <param name="HasWaterRipples">Water ripple toggle (<c>bUseWaterDisplacements:Water</c>; the
/// viewer maps it to the animated surface normal field — USER ruling 2026-08-18).</param>
/// <param name="HasWindowReflections">Window reflection toggle
/// (<c>bDynamicWindowReflections:Display</c>) — classic Gamebryo window env-map materials.</param>
/// <param name="HasGrassShadows">Grass shadow-receive toggle (<c>bShadowsOnGrass:Display</c>;
/// retail grass RECEIVES canopy shadows and never casts).</param>
/// <param name="HasCanopyShadows">Tree canopy shadow toggle (<c>bDoCanopyShadowPass:Display</c>;
/// the viewer's real cascaded shadow map stands in for retail's projected canopy blobs, so the
/// toggle gates tree casters).</param>
    ...
    bool HasCanopyShadows)
{
    ...
    internal static VideoSettingsProfile ForGame(BethesdaGame game) => game switch
    {
        ...
        BethesdaGame.Oblivion => new(
            HasTonemapSelector: true, HasClassicHdrTriple: true,
            HasWaterReflections: true, HasWaterRipples: true,
            HasWindowReflections: true, HasGrassShadows: true, HasCanopyShadows: true),
        ...
        BethesdaGame.Fallout3 or BethesdaGame.FalloutNewVegas => new(
            HasTonemapSelector: true, HasClassicHdrTriple: true,
            HasWaterReflections: true, HasWaterRipples: true,
            HasWindowReflections: true, HasGrassShadows: true, HasCanopyShadows: true),
        ...
        BethesdaGame.Skyrim or BethesdaGame.Fallout4 or BethesdaGame.Fallout76 => new(
            HasTonemapSelector: true, HasClassicHdrTriple: false,
            HasWaterReflections: true, HasWaterRipples: true,
            HasWindowReflections: false, HasGrassShadows: false, HasCanopyShadows: false),
        ...
        _ => new(
            HasTonemapSelector: false, HasClassicHdrTriple: false,
            HasWaterReflections: false, HasWaterRipples: false,
            HasWindowReflections: false, HasGrassShadows: false, HasCanopyShadows: false),
    };
}
