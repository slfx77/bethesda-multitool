...
internal static class NifVersions
{
    ...
    /// <summary>Legacy NetImmerse geometry-data layout (≤ 4.2.2.0, i.e. Morrowind 4.0.0.2): no Group ID,
    ...
    public static bool HasAvObjectVelocity(uint binaryVersion) => binaryVersion <= NetImmerse4220;

    /// <summary>NiAVObject has a Collision Object ref (since 10.0.1.0); older NIFs have a Has Bounding
    /// Volume flag + BoundingVolume union instead.</summary>
    public static bool HasCollisionObjectRef(uint binaryVersion) => binaryVersion >= Gamebryo10010;

    /// <summary>NiGeometryData begins with a Group ID int (since 10.1.0.114); Morrowind and Oblivion's
    /// 10.1.0.101 / 10.1.0.106 architecture omit it.</summary>
    public static bool HasGeometryGroupId(uint binaryVersion) => binaryVersion >= Gamebryo101114;

    /// <summary>NiGeometryData uses the "modern" Gamebryo base — a Data Flags ushort, an always-present
    /// Bounding Sphere, flag-driven UV sets and a trailing Consistency Flags — since 10.0.1.0. Below that
    /// (Morrowind 4.0.0.2) the geometry data has the older Data-Flags-after-colors + Has-UV layout.</summary>
    public static bool HasModernGeometryBase(uint binaryVersion) => binaryVersion >= Gamebryo10010;

    /// <summary>NiGeometryData carries Keep Flags + Compress Flags (two bytes, right after Num Vertices)
    ...
    /// vertex array and yields no geometry.</summary>
    public static bool HasGeometryKeepFlags(uint binaryVersion) => binaryVersion >= Gamebryo10100;

    /// <summary>NiTriShapeData writes a "Has Triangles" bool before its triangle list since 10.1.0.0; at or
    /// below 10.0.1.2 the triangle array follows Num Triangle Points unconditionally. Reading the bool when
    /// it isn't there consumes the first triangle index and (when zero) looks like "no triangles".</summary>
    public static bool HasShapeTriangleFlag(uint binaryVersion) => binaryVersion >= Gamebryo10100;

    /// <summary>NiTriStripsData writes a "Has Points" bool before its strip points since 10.0.1.3; at or
    /// below 10.0.1.2 (Oblivion's 10.0.1.0 / 10.0.1.2 meshes) the points follow the strip lengths
    /// unconditionally.</summary>
    public static bool HasStripPointsFlag(uint binaryVersion) => binaryVersion >= Gamebryo10013;

    /// <summary>Pre-10.2.0.0 Gamebryo/Bethesda streams prefix every data block with a 4-byte word that
    /// nif.xml does not model (observed on Oblivion 10.1.0.101 / 10.1.0.106). See
    /// <c>NifParser.MeasureLegacyBlocks</c>.</summary>
    public static bool HasPerBlockLegacyWord(uint binaryVersion) => binaryVersion < Gamebryo10200;

    /// <summary>The Header carries a User Version uint (since 10.0.1.8). The oldest Gamebryo NIFs Oblivion
    ...
    /// Getting this wrong reads block-type-table bytes as ExportStrings and loses all geometry.</summary>
