...
///     </para>
///     <para>
///         High byte <c>0xFF</c> is reserved for IDs that must stay <em>identical</em> across all
///         plugins so the merge folds them together rather than keeping them apart: the shared exterior
///         worldspace (<see cref="WorldspaceRecord.Tes3SyntheticExteriorFormId" />) and the land-texture
///         palette, which Morrowind keys by a global LTEX index. <see cref="Namespace" /> never rewrites
///         a 0xFF-prefixed ID, so a plugin can hold at most load index 0..254.
///     </para>
/// </summary>
internal static class Tes3FormIdScheme
{
    ...
    /// <summary>Base for synthetic LandscapeTexture (LTEX) FormIDs: shared base + Morrowind land-texture index.</summary>
    public const uint LtexFormIdBase = 0xFF100000u;

    /// <summary>Base for the synthetic TextureSet (TXST) backing each LTEX: shared base + land-texture index.</summary>
    public const uint LtexTextureSetFormIdBase = 0xFF200000u;
