...
///     FalloutNV.esm as the source of subrecord data the DMP doesn't carry. Reached through
///     <see cref="PluginConversionPipeline" />.
///     <para>Pipeline:</para>
...
public sealed class PluginBuilder
{
    ...
    public async Task<PluginBuildResult> BuildAsync(DmpToEsmInputs inputs, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var newWorldspacesForCellPipeline = PreEncodeNewWorldspacesWithCells(
                dmpRecords, classifier, allocator, inputs.Options);
            if (newWorldspacesForCellPipeline.Count > 0)
            {
                _sink.Info("Merging top-level records",
                    $"Deferred {newWorldspacesForCellPipeline.Count:N0} new WRLD(s) with child cells " +
        ...
        ConversionPipelineStats stats)
    {
        ...
        var referenceResolver = new BethesdaMultitool.Core.Formats.Esm.Planner.References.ReferenceResolver(
            new BethesdaMultitool.Core.Formats.Esm.Planner.References.IRecordReferenceWalker[]
            {
                ...
                new BethesdaMultitool.Core.Formats.Esm.Planner.References.Walkers.PerkReferenceWalker(),
            },
            degradationPolicy);
