...
///     <list type="bullet">
///         <item>FNV / FO3 ship NO fLeaf* GMST records or INI entries → compiled defaults
///         (Geck.exe initializers): timescales 2.0/0.5, all four influences 1.0
///         (⇒ amounts pulse with gustiness: <c>S·sway/2</c>, mean ≈ 0.64·S).</item>
///         <item>Oblivion.esm SHIPS them as GMSTs: amount influences 0.01 (⇒ amounts are a
...
///         the 1−k term negative); that is authentic engine behavior, not a bug.</item>
///     </list>
/// </summary>
    ...
    float RustleAmountSwayInfluence)
{
    ...
    public static SpeedTreeWindProfile FalloutNewVegas { get; } = new(
        RockTimeScale: 2.0f,
        RustleTimeScale: 0.5f,
        RockSpeedSwayInfluence: 1.0f,
        RustleSpeedSwayInfluence: 1.0f,
        RockAmountSwayInfluence: 1.0f,
        RustleAmountSwayInfluence: 1.0f);
    ...
    public static SpeedTreeWindProfile Oblivion { get; } = new(
        RockTimeScale: 1.0f,
        RustleTimeScale: 0.75f,
        RockSpeedSwayInfluence: 0.85f,
        RustleSpeedSwayInfluence: 1.5f,
        RockAmountSwayInfluence: 0.01f,
        RustleAmountSwayInfluence: 0.01f);
