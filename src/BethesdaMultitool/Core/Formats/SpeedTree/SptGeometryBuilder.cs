...
internal static class SptGeometryBuilder
{
        ...
        SptModel model, SptRandom rng, LeafPlacer placer)
    {
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            placer.TryPlace(rng, pos, budFrame, template, texCoords, budReach);
        }
    }
        ...
        List<PlacedLeaf> leaves, float cardScale, SptGeometryOptions opt, Dictionary<string, MeshBuffer> leafGroups)
    {
        ...
        foreach (var leaf in leaves)
        {
            ...
            // The .spt's token-10002 leaf UVs use a bottom-left texture origin (SpeedTree RT / the original
            // Gamebryo GL-style sampler); our rasterizer samples with a top-left origin, so V must be flipped
            // (v → 1−v) or every card samples the WRONG atlas region — leaf 0/1 grabbed both small top clusters
            // at once (a hard rectangular seam between them) and leaf 2/3 grabbed cut-off slices of the big
            // cluster (the squared texture edges). Flipped, each template resolves to one clean leaf cluster
            // (verified with `spt leaf-debug`: RAW vs FLIPPED-V columns). Bark UVs are generated by us and
            // already match our origin, so only the .spt-sourced leaf UVs need this.
            ...
            if (opt.LeafBillboard)
            {
                ...
                // frame. The lighting normal is the soft CANOPY normal, NOT budDir: with the bud's twig
                // direction as the normal, the shader's saturate(N·L) drove every leaf whose bud faced away
                // from the sun to near-black (only 0.3·ambient survives) — the "some leaves very dark" harshness.
                var (bbRight, bbUp) = BuildFrame(budDir);
                AddLeafCard(buffer, center, bbRight, bbUp, CanopyNormal(center), uv, x0, x1, y0, y1,
    ...
    private sealed class LeafPlacer
    {
        ...
        public void TryPlace(SptRandom rng, Vector3 pos, BranchFrame budFrame, SptLeaf template,
            SptLeafTextureCoords texCoords, float budReach)
        {
    ...
    /// canopy-height wind), the card center + facing direction, and which template / atlas UVs it uses.
    /// Geometry is built from these afterward by <see cref="EmitPlacedLeaves" />.</summary>
    private readonly record struct PlacedLeaf(
        Vector3 Pos, Vector3 Center, Vector3 BudDir, SptLeaf Template, SptLeafTextureCoords TextureCoords);
