...
public static class EsmStringUtils
{
    ...
    /// <summary>
    ///     Encode game-authored text as Windows-1252. This is the inverse of
    ///     <see cref="DecodeGameText" /> for every byte the game-text decoder accepts,
    ///     including the printable characters assigned to bytes 0x80-0x9F.
    /// </summary>
    public static byte[] EncodeGameText(ReadOnlySpan<char> text)
    {
        if (text.IsEmpty)
        {
            return [];
        ...
        {
            bytes[i] = EncodeWindows1252Char(text[i]);
        }
    ...
    private static byte EncodeWindows1252Char(char value)
    {
        return value switch
        {
            '\u20AC' => 0x80,
