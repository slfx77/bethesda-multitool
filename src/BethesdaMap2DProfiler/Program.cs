...
public static class Program
{
    ...
    {
        // Switches Map2DProfilerTrace on inside BethesdaMultitool — every cache mutation,
    ...
    private static void ConfigureLogger(Map2DProfilerOptions options)
    {
        ...
        try
        {
            if (!string.IsNullOrWhiteSpace(options.ProfileOutputPath))
            {
                ...
                Console.WriteLine($"[BethesdaMap2DProfiler] Profile log: {options.ProfileOutputPath}");
            }
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine($"[BethesdaMap2DProfiler] Failed to open profile log: {ex.Message}");
        }
    }
}
