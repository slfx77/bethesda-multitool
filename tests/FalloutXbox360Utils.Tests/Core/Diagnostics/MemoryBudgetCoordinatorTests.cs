...
public sealed class MemoryBudgetCoordinatorTests
{
    ...
    public void No_trims_when_under_budget_and_gc_calm()
    {
        ...
        var participant = new FakeParticipant("A", 10 * OneMb, 10);
        registry.Register(participant);

        var coordinator = new MemoryBudgetCoordinator(registry, 100 * OneMb, static () => 0.1);
    ...
    public void Over_budget_trims_gently_in_priority_order_and_stops_at_target()
    {
        ...
        var first = new FakeParticipant("first", 100 * OneMb, 10);
        var second = new FakeParticipant("second", 20 * OneMb, 20);
        var third = new FakeParticipant("third", 20 * OneMb, 30);
        registry.Register(third);
        registry.Register(first);
        registry.Register(second);

        var coordinator = new MemoryBudgetCoordinator(registry, 80 * OneMb, static () => 0.1);
        coordinator.CheckNow("test");
    ...
    {
        var registry = new ResourceRegistry();
        var ownerThread = new FakeParticipant("owner", 200 * OneMb, 10, TrimAffinity.OwnerThread);
    ...
    public void Gc_pressure_alone_under_budget_does_not_trim()
    {
        ...
        var registry = new ResourceRegistry();
        var first = new FakeParticipant("first", 1 * OneMb, 10);
        var second = new FakeParticipant("second", 1 * OneMb, 20);
        registry.Register(first);
        registry.Register(second);

        var coordinator = new MemoryBudgetCoordinator(registry, 100 * OneMb, static () => 0.95);
        coordinator.CheckNow("test");
    ...
    public void Over_budget_under_gc_pressure_trims_aggressively_over_every_participant()
    {
        ...
        var coordinator = new MemoryBudgetCoordinator(registry, 80 * OneMb, static () => 0.95);
        coordinator.CheckNow("test");
    ...
    public void No_cap_budget_never_trims_even_with_large_caches_under_gc_pressure()
    {
        ...
        var registry = new ResourceRegistry();
        var big = new FakeParticipant("big", 8L * 1024 * OneMb, 10); // 8 GB
        registry.Register(big);

        var coordinator = new MemoryBudgetCoordinator(
            registry, long.MaxValue, static () => 0.99);
        coordinator.CheckNow("test");
    ...
    {
        var registry = new ResourceRegistry();
        var participant = new FakeParticipant("A", 500 * OneMb, 10);
        registry.Register(participant);

        var coordinator = new MemoryBudgetCoordinator(
            registry, OneMb, static () => 0.99, true);
        coordinator.CheckNow("test");
        coordinator.Start();
        ...
        TrimAffinity affinity = TrimAffinity.AnyThread) : IMemoryPressureParticipant
    {
        ...
        {
            return new ResourceStats { EstimatedBytes = Bytes };
        }
    }
}