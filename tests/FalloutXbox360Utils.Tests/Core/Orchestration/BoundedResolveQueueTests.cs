...
public sealed class BoundedResolveQueueTests
{
    ...
    public void Enqueue_dedupes_until_the_completion_is_dequeued()
    {
        ...
        var queue = new BoundedResolveQueue<string, string>(
            "TestQueue", maxConcurrent: 4, resolve: static key => key + "!",
            keyComparer: StringComparer.OrdinalIgnoreCase, scheduler: scheduler.Schedule);
    ...
    public void Pump_starts_at_most_max_concurrent_resolutions()
    {
        var scheduler = new DeferredScheduler<string>();
        var queue = new BoundedResolveQueue<string, string>(
            "TestQueue", maxConcurrent: 2, resolve: static key => key, scheduler: scheduler.Schedule);
    ...
    public void Faulted_resolution_yields_a_null_result_completion()
    {
        var scheduler = new DeferredScheduler<string>();
        var queue = new BoundedResolveQueue<string, string>(
            "TestQueue", maxConcurrent: 1,
            resolve: static _ => throw new InvalidOperationException("decode failed"),
    ...
    public void WaitForDrain_blocks_until_in_flight_resolutions_finish()
    {
        ...
        var queue = new BoundedResolveQueue<string, string>(
            "TestQueue", maxConcurrent: 1, resolve: key =>
            {
                gate.Wait();
                return key;
            });
...
internal static class BoundedResolveQueueTestExtensions
{
    ...
    public static bool TryDequeueCompletedProbe(this BoundedResolveQueue<string, string> queue) =>
        queue.TryDequeueCompleted(out _, out _);
}
