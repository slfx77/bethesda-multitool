...
public sealed class AlternateTextureSetTests
{
    private static KeyValuePair<string, ShapeTextureOverride> Entry(string shape, string? diffuse,
        string? normal = null)
    {
        return new KeyValuePair<string, ShapeTextureOverride>(shape, new ShapeTextureOverride(diffuse, normal));
    }
    ...
    public void Create_NoEffectiveOverrides_ReturnsNull()
    {
        ...
        Assert.Null(AlternateTextureSet.Create([Entry("BB04:13", null)]));
    }
