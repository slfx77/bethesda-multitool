...
///     NifKeyGroupReader stride table on real 4.0.0.2 data), the text keys must resolve the
///     Idle3 loop window (2.333–3.667 s), the rest pose must reproduce the authored bone offsets,
...
public class Tes3BannerAnimationProbe
{
    ...
    public void Banner_CollectsRig_Clip_And_Skin()
    {
        ...
        // Clip = the Idle3 loop window.
        Assert.Equal(2.333f, animation.ClipStart, 2);
        Assert.Equal(3.667f, animation.ClipStop, 2);
